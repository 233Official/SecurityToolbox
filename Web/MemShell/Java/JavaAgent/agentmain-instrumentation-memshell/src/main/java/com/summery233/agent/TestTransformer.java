...
public class TestTransformer implements ClassFileTransformer{
    ...
    private static void dumpClass(String className, byte[] classfileBuffer) throws IOException {
        ...
        if (Pattern.compile(regexp).matcher(className).find()) {
            ...
            // FileOutputStream("/Users/phoebe/IdeaProjects/MemoryShell/" +
            // "memshell-inject/target/" + className + ".class");

            // FileOutputStream fos = new FileOutputStream(
            // "/usr/local/tomcat/webapps/ROOT/" + "memshell-inject/target/"+ className +
            // ".class");

            FileOutputStream fos = new FileOutputStream(
                    "C:/Software/memshell-inject" + "memshell-inject/target/" + className + ".class");
